...
Backend Terminal is an **App for Agent Canvas** with a bundled xterm.js frontend and a real PTY shell on the sidecar host. The checked-in Canvas `extension.js` contains the complete terminal UI, a checksummed server-only sidecar artifact, and onboarding; the installed sidecar carries input, output, resize, and lifecycle events over WebSocket.

## Architecture

1. Canvas Blob-imports the self-contained `extension.js` and mounts the Terminal page.
2. The extension renders xterm.js directly in a shadow root; it does not load an iframe or hosted SPA.
3. The extension resolves the host backend ID and `openhands-active-backend` against Canvas-owned `openhands-backends` localStorage, then requests a short-lived, one-use capability from the sidecar using exact-origin CORS.
4. The extension opens `/api/terminal` and sends the capability and active local backend key only in its first WebSocket message.
5. The sidecar checks both HTTP and WebSocket origins against `TERMINAL_ALLOWED_ORIGINS`, validates the key live against its loopback Agent Server, then spawns a PTY. Closing the socket kills the shell.
...
- `extension.js`: self-contained Canvas entrypoint with xterm.js and CSS, copied from `dist/extension.js`.
- `sidecar-dist/server/`: Node HTTP, WebSocket, and PTY backend.

`npm run check` performs strict type checking, unit/integration tests including first-run UI provisioning, localStorage selection, exact-origin CORS, non-root startup, live key validation, and a real PTY session, all builds, Canvas static validation, and two Chromium smoke tests. The additional networked clean-install integration test is opt-in through `npm run smoke:install`.


## First-run local provisioning

Opening the App first performs a read-only probe through the active local backend's authenticated Agent Server. A healthy matching sidecar opens directly into the terminal. A missing or incompatible runtime shows a setup view instead of opening a failing terminal connection.

After explicit confirmation, **Install and start** safely stops a prior PID-file-owned App sidecar, writes the bundled, per-file SHA-256-verified runtime to `<backend-home>/.openhands/apps/backend-terminal`, runs `npm ci --omit=dev` against the pinned production lockfile, and launches a process as the same operating-system user that runs the Agent Server command. A compatible stopped runtime presents a separate **Start sidecar** action and does not reinstall. The App waits for versioned health before mounting the bundled terminal.
...
| `TERMINAL_ALLOWED_ORIGINS` | local Canvas origins on port 8000 | Comma-separated exact Canvas origins allowed for capability CORS and terminal WebSocket upgrades. |
...
- Capability CORS and WebSocket upgrades require an exact origin in `TERMINAL_ALLOWED_ORIGINS`; denied origins receive no CORS allow header.
- The bundled extension reads the selected local backend key only from Canvas-owned `openhands-active-backend` and `openhands-backends` localStorage records and requires it to match `host.backend.id`. It keeps the key in memory only until the WebSocket authentication frame; the sidecar never persists or logs it and never passes it to the PTY environment.
- Capability requests omit ambient browser credentials and never contain the backend key.
...
- xterm.js and its styles are bundled into `extension.js`; terminal styles are isolated in a shadow root and no hosted UI is executed.
